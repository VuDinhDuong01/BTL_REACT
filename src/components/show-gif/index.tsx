...
export const ShowGIF = forwardRef<handleShowPopup, Pops>(({ limit }, ref) => {
    ...
    return <div>
        {
            isShowPopup && <Dialog open={isShowPopup}>
                ...
                <div className='w-full h-full !z-[99] flex fixed top-[-100px] bottom-0 left-0 right-0 items-center justify-center'>
                    <div className='h-[600px] min-w-[450px]  !z-[9] bg-white rounded-[20px] flex flex-col items-center' style={{ boxShadow: "0px 4px 20px 0px rgba(0, 0, 0, 0.15)" }}>
                        ...
                        <div className='w-full grid grid-cols-12 gap-1 overflow-y-scroll'>
                            {
                                getListGif && getListGif.map(gif => {
                                    return <div key={gif.id} className='w-full  col-span-4 '>
                                        <img src={gif.url} alt='gif' className='w-full h-[100px] object-cover cursor-pointer' />
                                    </div>
                                })
                            }
                        </div>
